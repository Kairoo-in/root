...
- `deep`: Gemini `gemini-2.5-pro` → (Claude `claude-opus-4-8` *if key set*)
...
- **anthropic.ts** — Claude; enabled iff `ANTHROPIC_API_KEY` (off by default). *(When
  implementing, consult the claude-api skill for correct model IDs/SDK usage.)*
